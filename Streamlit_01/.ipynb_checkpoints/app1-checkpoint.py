...
# Introduction
st.markdown("<h2 style='text-align: center;'>Prévision de la viscosité dynamique de la bouillie utilisant le machine learning</h2>", unsafe_allow_html=True)

def load_model_regression(degree):
    model_path = f'rg_poly_degree_{degree}.pkl'
    return pickle.load(open(model_path, 'rb'))

def load_ann_adam(nb_neurons,fct_activation):
    model_path = f'ANN_adam_{nb_neurons}_{fct_activation}.pkl'
    return pickle.load(open(model_path, 'rb'))

def load_ann_sgd(nb_neurons,fct_activation):
    model_path = f'ANN_sgd_{nb_neurons}_{fct_activation}.pkl'
    return pickle.load(open(model_path, 'rb'))
...
# Affichage des résultats
  ...
  if optimizer=="Adam":
      if st.sidebar.button("Execution"):
        ...
        model1 = load_ann_adam(nombre_neurones,fonction_activation)
      ...
      if st.sidebar.button("Execution"):
        #chargement des modèles
        model2 = load_ann_sgd(nombre_neurones,fonction_activation)
        #Metiques d'évaluation
  ...
  if st.sidebar.button("Execution"):
      ...
      st.markdown("<p style='text-align: center;'>Résultats obtenues:</p>", unsafe_allow_html=True)
      st.markdown(html_tableau, unsafe_allow_html=True)
      fig = px.scatter(data,x=y_test,y=y_pred,labels={'x':"viscosité réelle",
                                                      'y':"viscosité prédite"})
...
if st.sidebar.button("Méthodologie"):
   ...
   #fig = px.box(data,labels=['x1', 'x2', 'x3','y'],
                #title='figure 1: Boîte à moustaches pour chaque paramètre')
